...
TWR.Renderers = (function() {
    ...
    function toggleIssueExpand(index, event) {
        ...
        if (isExpanded && !expandedDiv) {
            ...
            if (issue.flagged_text || issue.context) {
                ...
                let contextHtml = escapeHtml(context || flagged);
                
                if (flagged && context.includes(flagged)) {
                    contextHtml = escapeHtml(context).replace(
                        escapeHtml(flagged),
    ...
    function renderSectionHeatmap(issues, totalParagraphs) {
        ...
        // Divide document into sections (max 20 sections)
        const numSections = Math.min(20, Math.max(5, Math.ceil(totalParagraphs / 10)));
        ...
        // Build heatmap HTML
        const cells = sectionCounts.map((count, idx) => {
            ...
            const tooltip = `Paragraphs ${startPara}-${endPara}: ${count} issue${count !== 1 ? 's' : ''}`;
            
            return `<div class="heatmap-cell ${densityClass}" 
                         title="${tooltip}"
                         data-section="${idx}"
        ...
        container.innerHTML = recommendations.map((rec, idx) => `
            <div class="recommendation-item priority-${rec.priority}">
                ...
                <div class="rec-action">
                    ${rec.action ? `<button class="btn btn-xs btn-primary" onclick="${rec.action}">${rec.actionText}</button>` : ''}
                </div>
            </div>
        `).join('');
                        ...
                        ${roleSummary.top_roles && roleSummary.top_roles.length > 0 ? `
                            <div class="top-roles">
                                ${roleSummary.top_roles.slice(0, 3).map(r => 
                                    `<span class="role-chip">${r.name} (${r.count})</span>`
                                ).join('')}
                            </div>
                        ` : ''}
                    </div>
